            ...
            <main className="flex flex-col min-h-screen px-6 sm:px-12 md:px-16 lg:px-25 pt-32 md:pt-40 py-16 md:py-32 animate-page-fade-in">
                ...
                <div className="flex flex-col md:flex-row items-start gap-8 md:gap-16 lg:gap-35">
                    ...
                    <div className="flex flex-row md:flex-col gap-6 md:gap-2 w-full md:w-32 shrink-0">
                        <button
                            onClick={() => setYear(1)}
                            className={`uppercase tracking-wider font-helvetica cursor-pointer font-bold hover:text-ara-white text-md text-left ${year === 1 ? 'text-ara-white' : 'text-chromeangels-grey'}`}
                        >
                            Year One
                        </button>
                        <button
                            onClick={() => setYear(2)}
                            className={`uppercase tracking-wider font-helvetica cursor-pointer font-bold hover:text-ara-white text-md text-left ${year === 2 ? 'text-ara-white' : 'text-chromeangels-grey'}`}
                        >
                            Year Two
                        </button>
                    </div>
                    ...
                    <div className="columns-2 md:columns-3 lg:columns-3 xl:columns-4 gap-x-8 lg:gap-x-35 w-full flex-wrap">
                        {ArtistList
                            ...
                            .map(artist => (
                                <h3
                                    key={artist.artistId}
                                    className="font-helvetica text-ara-white uppercase tracking-wider font-bold text-md mb-2 break-inside-avoid"
                                >
                                    {artist.artistName}
                                </h3>
                            ))
